...
public class MvcConfig implements WebMvcConfigurer {
	...
	public SqlSessionFactory sqlSessionFactory() throws Exception {
		SqlSessionFactoryBean ssfd = new SqlSessionFactoryBean();
		ssfd.setDataSource(dataSource());
		
		// sql이 들어있는 xml경로 설정
		PathMatchingResourcePatternResolver resolver = new PathMatchingResourcePatternResolver();
		ssfd.setMapperLocations(resolver.getResources("classpath:/mapper/**/*.xml")); // 클래스패스의 mapper폴더 밑의 모든 xml
		return ssfd.getObject();
	}
	...
	public CommonsMultipartResolver multipartResolver() {
		...
		cmr.setMaxUploadSize(1024*1024*10); // 최대업로드사이즈(바이트=10메가)
		return cmr;
	}
}
